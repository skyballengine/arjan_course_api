...
def update_booking(booking_id: int, booking_data: UpdateBookingData, booking_interface: DataInterface) -> DataObject:
    #create room variable if below if statement fails
    room_interface = DBInterface(DBRoom)
    room = room_interface.read_by_id(booking_data.room_id)

    # find (and then overwrite) room to change booking price if room changes and room price is different
    if not booking_data.room_id == booking_interface.read_by_id(booking_id).get("room_id"):
        room_interface = DBInterface(DBRoom)
        room = room_interface.read_by_id(booking_data.room_id)
...
def read_availability_by_date_range(date_range: str, booking_interface: DataInterface, room_interface: DataInterface) -> DataObject:
    ...
    print(f"The following rooms are unavailable: {unavailable_rooms}")
